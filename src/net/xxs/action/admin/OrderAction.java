...
import net.xxs.entity.Refund.RefundType;
...
public class OrderAction extends BaseAdminAction {
	...
	public String update() {
		...
		for (OrderItem orderItem : orderItemList) {
			...
			if (product.getStore() != null) {
				...
				if ((setting.getStoreFreezeTime() == StoreFreezeTime.payment && order.getPaymentStatus() == net.xxs.entity.Order.PaymentStatus.unpaid) || 
				...
				if (setting.getStoreFreezeTime() == StoreFreezeTime.order || (setting.getStoreFreezeTime() == StoreFreezeTime.payment && order.getPaymentStatus() != net.xxs.entity.Order.PaymentStatus.unpaid) || 
					(setting.getStoreFreezeTime() == StoreFreezeTime.ship && order.getShippingStatus() != ShippingStatus.unshipped)) {
	...
	public String payment() {
		...
		// 库存占用数处理
		Setting setting = getSetting();
		if (setting.getStoreFreezeTime() == StoreFreezeTime.payment && order.getPaymentStatus() == net.xxs.entity.Order.PaymentStatus.unpaid && order.getShippingStatus() == ShippingStatus.unshipped) {
		...
		for (DeliveryItem deliveryItem : deliveryItemList) {
			...
			for (OrderItem orderItem : orderItemSet) {
				if (product == orderItem.getProduct()) {
					...
					if (product.getStore() != null) {
						if ((setting.getStoreFreezeTime() == StoreFreezeTime.payment && order.getPaymentStatus() == net.xxs.entity.Order.PaymentStatus.unpaid && order.getShippingStatus() == ShippingStatus.unshipped) || (setting.getStoreFreezeTime() == StoreFreezeTime.ship && order.getShippingStatus() == ShippingStatus.unshipped)) {
	...
	public String refund() {
		...
		if (order.getPaymentStatus() == net.xxs.entity.Order.PaymentStatus.unpaid || order.getPaymentStatus() == net.xxs.entity.Order.PaymentStatus.refunded) {
		...
		order.setOrderStatus(OrderStatus.processed);
		if (refund.getTotalAmount().compareTo(order.getPaidAmount()) < 0) {
			order.setPaymentStatus(net.xxs.entity.Order.PaymentStatus.partRefund);
		} else {
			order.setPaymentStatus(net.xxs.entity.Order.PaymentStatus.refunded);
		}
	...
	public String invalid() {
		...
		if (order.getOrderStatus() == OrderStatus.completed) {
			...
			Setting setting = getSetting();
			if (setting.getStoreFreezeTime() == StoreFreezeTime.order || (setting.getStoreFreezeTime() == StoreFreezeTime.payment && order.getPaymentStatus() != net.xxs.entity.Order.PaymentStatus.unpaid) || order.getShippingStatus() != ShippingStatus.unshipped) {
				for (OrderItem orderItem : order.getOrderItemSet()) {
					Product product = orderItem.getProduct();
					product.setFreezeStore(product.getFreezeStore() - orderItem.getProductQuantity());
					productService.update(product);
